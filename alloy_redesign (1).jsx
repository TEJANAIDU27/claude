...
function calcCriticalTemps(comp) {
  const { C: carbon = 0, Mn = 0, Ni = 0, Cr = 0, Mo = 0, Si = 0, V = 0 } = comp;
  const Ac1 = 723 - 16.9 * Ni + 29.1 * Si + 16.9 * Cr - 10.7 * Mn - 16.9 * Mo;
  const Ac3 = 910 - 203 * Math.sqrt(carbon) - 15.2 * Ni + 44.7 * Si + 104 * V + 31.5 * Mo - 30 * Mn - 11 * Cr;
  return { Ac1: Math.round(Ac1), Ac3: Math.round(Ac3) };
}
...
const Badge = ({ pass, children }) => (
  <span style={{
    ...
    background: pass ? COLORS.emeraldLight : COLORS.roseLight,
    color: pass ? COLORS.emeraldDark : COLORS.roseDark,
    border: `1px solid ${pass ? COLORS.emerald : COLORS.rose}`,
  }}>{children}</span>
);
...
const Card = ({ children, style = {} }) => (
  <div style={{
    ...
    border: `1px solid ${COLORS.slate200}`, ...style
  }}>{children}</div>
);
...
const SectionTitle = ({ icon, children }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
    ...
    <h3 style={{ margin: 0, fontSize: 13, fontWeight: 700, letterSpacing: "0.08em",
      textTransform: "uppercase", color: COLORS.slate700 }}>{children}</h3>
  </div>
);

const MetricCard = ({ label, value, unit, pass, delta }) => (
  <div style={{
    background: COLORS.slate100, borderRadius: 10, padding: "14px 16px",
    border: `1px solid ${COLORS.slate200}`,
  }}>
    <div style={{ fontSize: 11, color: COLORS.slate, fontWeight: 600, textTransform: "uppercase",
      letterSpacing: "0.06em", marginBottom: 4 }}>{label}</div>
    <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
      <span style={{ fontSize: 24, fontWeight: 800, color: COLORS.slate900, fontFamily: "'IBM Plex Mono', monospace" }}>{value}</span>
      <span style={{ fontSize: 12, color: COLORS.slate, fontWeight: 500 }}>{unit}</span>
    </div>
    {delta !== undefined && (
      <div style={{ fontSize: 11, marginTop: 3, color: delta >= 0 ? COLORS.emeraldDark : COLORS.roseDark, fontWeight: 600 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        <span style={{ fontSize: 11, color: COLORS.slate, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>{label}</span>
        <span style={{ fontSize: 13, fontWeight: 700, color: COLORS.slate900, fontFamily: "'IBM Plex Mono', monospace" }}>{Math.round(pct)}%</span>
      </div>
      <div style={{ height: 7, borderRadius: 99, background: COLORS.slate200, overflow: "hidden" }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderBottom: `3px solid ${COLORS.emerald}`,
        padding: "0 32px",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{
            ...
            background: `linear-gradient(135deg, ${COLORS.emerald}, #0ea5e9)`,
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 16,
          }}>⚛</div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: "#fff", letterSpacing: "-0.02em" }}>
              AlloyOS <span style={{ color: COLORS.emerald }}>Research</span>
            </div>
            <div style={{ fontSize: 10, color: COLORS.slate, letterSpacing: "0.12em", textTransform: "uppercase" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={handleExport} style={{
            ...
            padding: "7px 16px", borderRadius: 8, border: `1px solid ${COLORS.emerald}`,
            background: "transparent", color: COLORS.emerald, fontSize: 12, fontWeight: 700,
            cursor: "pointer", letterSpacing: "0.04em", transition: "all 0.2s",
          }}
            onMouseEnter={e => e.target.style.background = COLORS.emerald + "22"}
      ...
      {/* ── NAV TABS ── */}
      <div style={{ background: COLORS.white, borderBottom: `1px solid ${COLORS.slate200}`, padding: "0 32px", display: "flex", gap: 0 }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{
            padding: "14px 20px", background: "none", border: "none", cursor: "pointer",
            fontSize: 13, fontWeight: activeTab === t.id ? 700 : 500,
            color: activeTab === t.id ? COLORS.emeraldDark : COLORS.slate,
            borderBottom: `3px solid ${activeTab === t.id ? COLORS.emerald : "transparent"}`,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: 20 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <Card>
                ...
                <input value={alloyName} onChange={e => setAlloyName(e.target.value)} style={{
                  width: "100%", padding: "10px 12px", borderRadius: 8, border: `1px solid ${COLORS.slate200}`,
                  fontSize: 14, fontWeight: 600, color: COLORS.slate900, background: COLORS.slate100,
                  outline: "none", boxSizing: "border-box",
                }} />
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontSize: 11, color: COLORS.slate, fontWeight: 700, letterSpacing: "0.07em",
                  ...
                  <input type="number" value={targetYS} onChange={e => setTargetYS(+e.target.value)} style={{
                    width: "100%", padding: "8px 12px", borderRadius: 8, border: `1px solid ${COLORS.slate200}`,
                    fontSize: 14, fontWeight: 600, color: COLORS.slate900, background: COLORS.slate100,
                    outline: "none", boxSizing: "border-box",
                  }} />
                </div>
                  ...
                  {elements.map(el => (
                    <div key={el} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{
                        ...
                        color: el === "Fe" ? COLORS.blue : COLORS.slate900,
                        flexShrink: 0,
                      }}>{el}</span>
                      <input
                      ...
                      <input
                        ...
                        step={el === "C" ? 0.01 : 0.1}
                        style={{
                          width: 52, padding: "4px 6px", borderRadius: 6, border: `1px solid ${COLORS.slate200}`,
                          fontSize: 12, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600,
                          color: COLORS.slate900, background: el === "Fe" ? COLORS.slate100 : COLORS.white,
                          outline: "none", textAlign: "right",
                        }}
                      />
                      <span style={{ fontSize: 10, color: COLORS.slate, width: 14 }}>%</span>
                    </div>
                  ))}
                </div>
                <div style={{
                  marginTop: 12, padding: "8px 12px", borderRadius: 8,
                  background: Math.abs(Object.values(normComp).reduce((a, b) => a + b, 0) - 100) < 0.5 ? COLORS.emeraldLight : COLORS.roseLight,
                  fontSize: 11, color: COLORS.slate700, fontWeight: 600, textAlign: "center",
                }}>
              ...
              <Card>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    {quenchOptions.map(q => (
                      <button key={q} onClick={() => setQuench(q)} style={{
                        flex: 1, padding: "8px 0", borderRadius: 8, border: `1.5px solid ${quench === q ? COLORS.emerald : COLORS.slate200}`,
                        background: quench === q ? COLORS.emeraldLight : COLORS.white,
                        color: quench === q ? COLORS.emeraldDark : COLORS.slate,
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 11, color: COLORS.slate, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em" }}>
                      Grain Size (μm)
                    </span>
                  ...
                  <input type="range" min={2} max={100} value={grainSize} onChange={e => setGrainSize(+e.target.value)}
                    style={{ width: "100%", accentColor: COLORS.emerald }} />
                </div>
              </Card>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
                {/* Hardenability */}
                <Card style={{ background: results.hardenability.prob > 0.5 ? COLORS.emeraldLight : COLORS.roseLight }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase",
                    color: results.hardenability.prob > 0.5 ? COLORS.emeraldDark : COLORS.roseDark, marginBottom: 8 }}>
                    Hardenability
                  </div>
                  <div style={{ fontSize: 36, fontWeight: 900, color: results.hardenability.prob > 0.5 ? COLORS.emeraldDark : COLORS.roseDark,
                    fontFamily: "'IBM Plex Mono', monospace", lineHeight: 1 }}>
                    {(results.hardenability.prob * 100).toFixed(0)}<span style={{ fontSize: 16 }}>%</span>
                  </div>
                  <div style={{ fontSize: 12, marginTop: 4, color: COLORS.slate700 }}>
                    CE = {results.CE.toFixed(3)} — {results.hardenability.label}
                  </div>
                ...
                <Card>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase",
                    color: COLORS.slate, marginBottom: 8 }}>Sustainability</div>
                  <div style={{ fontSize: 36, fontWeight: 900, color: results.sustainability > 60 ? COLORS.emeraldDark : COLORS.amber,
                    fontFamily: "'IBM Plex Mono', monospace", lineHeight: 1 }}>
                  ...
                  <div style={{ marginTop: 10 }}>
                    <Gauge value={results.sustainability} max={100} label="Eco Score"
                      color={results.sustainability > 60 ? COLORS.emerald : COLORS.amber} />
                  </div>
                </Card>

                {/* Phase Stability */}
                <Card style={{ background: results.phase.stable ? COLORS.white : COLORS.amberLight }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase",
                    color: COLORS.slate, marginBottom: 8 }}>Phase Stability</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: results.phase.stable ? COLORS.emeraldDark : COLORS.amber,
                  ...
                  {results.phase.warnings.length === 0 ?
                    ...
                    results.phase.warnings.map((w, i) => (
                      <div key={i} style={{ fontSize: 11, color: COLORS.amber, marginTop: 4, lineHeight: 1.5 }}>{w}</div>
                    ))
                  }
                </Card>
              </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
                  {[
                    ...
                    <div key={m.label} style={{
                      background: COLORS.slate900, borderRadius: 10, padding: "12px 14px",
                    }}>
              ...
              <Card>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  <Gauge value={results.YS} max={Math.max(1200, results.YS)} label="Yield Strength" color={COLORS.emerald} />
                  <Gauge value={results.vfMartensite * 100} max={100} label={`Martensite Vf (${quench})`} color={COLORS.indigo} />
                  <Gauge value={results.hardenability.prob * 100} max={100} label="Quench Probability" color={COLORS.blue} />
                  <Gauge value={results.sustainability} max={100} label="Sustainability Score" color={COLORS.amber} />
                </div>
              </Card>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20 }}>
            <Card>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                {quenchOptions.map(q => (
                  <button key={q} onClick={() => setQuench(q)} style={{
                    padding: "6px 14px", borderRadius: 8, border: `1.5px solid ${quench === q ? COLORS.emerald : COLORS.slate200}`,
                    background: quench === q ? COLORS.emeraldLight : COLORS.white,
                    color: quench === q ? COLORS.emeraldDark : COLORS.slate,
              ...
              <ResponsiveContainer width="100%" height={420}>
                <LineChart data={tttChartData} margin={{ top: 10, right: 30, left: 20, bottom: 40 }}>
                  ...
                  <YAxis domain={[100, 750]} label={{ value: "Temperature (°C)", angle: -90,
                    position: "insideLeft", offset: 10, style: { fontSize: 11, fill: COLORS.slate } }} />
                  <Tooltip formatter={(v, n) => [typeof v === "number" ? v.toFixed(1) : v, n]}
                    labelFormatter={l => `Time: ${l}s`}
                    contentStyle={{ fontSize: 12, borderRadius: 8 }} />
                  <Legend verticalAlign="top" height={36} />
                  <ReferenceLine y={results.ttt.noseTemp} stroke={COLORS.amber} strokeDasharray="5 3"
                    label={{ value: `Nose: ${results.ttt.noseTemp}°C`, fill: COLORS.amber, fontSize: 11 }} />
                  <ReferenceLine y={results.Ms} stroke={COLORS.rose} strokeDasharray="5 3"
                    label={{ value: `Ms: ${results.Ms}°C`, fill: COLORS.rose, fontSize: 11 }} />
                  <Line dataKey="ttt" name="TTT Boundary" stroke={COLORS.blue} strokeWidth={2.5}
                    dot={false} connectNulls activeDot={{ r: 4 }} />
                  <Line dataKey="cooling" name={`${quench} Cooling Path`}
                    stroke={quench === "Water" ? COLORS.emerald : quench === "Oil" ? COLORS.amber : COLORS.rose}
                ...
                {[
                  ...
                  <div key={m.l} style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ fontSize: 26, fontWeight: 800, fontFamily: "'IBM Plex Mono', monospace",
                      color: COLORS.slate900 }}>{m.v}°C</div>
                    <div style={{ fontSize: 11, color: COLORS.slate }}>{m.hint}</div>
                  </div>
                ))}
              </Card>
              <Card>
                <SectionTitle icon="❄️">Martensite Transformation</SectionTitle>
                <div style={{ fontSize: 11, color: COLORS.slate, marginBottom: 8 }}>
                  Koistinen-Marburger: Vf = 1 − exp(−α·(Ms−T))
                </div>
                <div style={{ fontSize: 32, fontWeight: 900, color: results.vfMartensite > 0.7 ? COLORS.emeraldDark : COLORS.amber,
                ...
                <div style={{ marginTop: 12 }}>
                  <Gauge value={results.vfMartensite * 100} max={100} label="Vf" color={COLORS.indigo} />
                </div>
              </Card>
            </div>
          </div>
              ...
              <ScatterChart margin={{ top: 20, right: 60, bottom: 60, left: 40 }}>
                ...
                <XAxis dataKey="cost" type="number" scale="log" domain={[0.1, 100]}
                  ...
                  label={{ value: "Cost per kg (USD, log scale)", position: "insideBottom", offset: -30,
                    style: { fontSize: 12, fill: COLORS.slate } }} />
                <YAxis dataKey="stw" type="number" scale="log" domain={[50, 400]}
                  ...
                  label={{ value: "Specific Strength (MPa·cm³/g)", angle: -90,
                    position: "insideLeft", offset: -10, style: { fontSize: 12, fill: COLORS.slate } }} />
                <Tooltip
                  ...
                  content={({ payload }) => {
                    ...
                    return (
                      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.slate200}`, borderRadius: 10,
                        padding: "12px 16px", boxShadow: "0 4px 16px rgba(0,0,0,0.1)" }}>
                        <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6, color: d.isMain ? COLORS.emeraldDark : COLORS.slate900 }}>
                          {d.name}
                        </div>
                        <div style={{ fontSize: 12, color: COLORS.slate }}>Specific Strength: {d.stw} MPa·cm³/g</div>
                        <div style={{ fontSize: 12, color: COLORS.slate }}>Cost: ${typeof d.cost === "number" ? d.cost.toFixed(2) : d.cost}/kg</div>
                      </div>
                    );
                  }}
                />
                <Scatter
                  ...
                  name="Reference Alloys" fill={COLORS.slate}
                  shape={(props) => {
                    const { cx, cy, payload } = props;
                    return (
                      <g>
                        <circle cx={cx} cy={cy} r={7} fill={COLORS.slate200} stroke={COLORS.slate} strokeWidth={1.5} />
                        <text x={cx + 10} y={cy + 4} fontSize={10} fill={COLORS.slate} fontWeight={600}>{payload.name}</text>
                      </g>
                    );
                  }}
                />
                <Scatter
                  data={ashbyData.filter(d => d.isMain)}
                  name={alloyName} fill={COLORS.emerald}
                  shape={(props) => {
                    const { cx, cy, payload } = props;
                    return (
                      <g>
                        <circle cx={cx} cy={cy} r={14} fill={COLORS.emeraldLight} stroke={COLORS.emerald} strokeWidth={2.5} />
                        <circle cx={cx} cy={cy} r={6} fill={COLORS.emerald} />
                        <text x={cx + 18} y={cy + 4} fontSize={11} fill={COLORS.emeraldDark} fontWeight={800}>{payload.name}</text>
                      </g>
                    );
                  }}
                />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginTop: 16 }}>
              {ashbyData.map(d => (
                <div key={d.name} style={{
                  ...
                  background: d.isMain ? COLORS.emeraldLight : COLORS.slate100,
                  border: `1px solid ${d.isMain ? COLORS.emerald : COLORS.slate200}`,
                }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: d.isMain ? COLORS.emeraldDark : COLORS.slate900 }}>{d.name}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  {results.narrative.map((line, i) => (
                    <div key={i} style={{
                      ...
                      background: COLORS.slate100, borderRadius: 10, border: `1px solid ${COLORS.slate200}`,
                    }}>
                      <div style={{
                        width: 28, height: 28, borderRadius: 8, flexShrink: 0,
                        background: COLORS.emerald, display: "flex", alignItems: "center",
                        justifyContent: "center", color: "#fff", fontSize: 13, fontWeight: 800,
                      }}>{i + 1}</div>
                      <div style={{ fontSize: 13, color: COLORS.slate700, lineHeight: 1.7 }}>{line}</div>
                    </div>
                  ))}
                </div>
              </Card>
              ...
              <Card>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                  {[
                    ...
                    <div key={m.l} style={{ background: COLORS.slate100, borderRadius: 10, padding: "14px 16px",
                      border: `1px solid ${COLORS.slate200}` }}>
                      <div style={{ fontSize: 10, color: COLORS.slate, fontWeight: 700, textTransform: "uppercase",
                        letterSpacing: "0.06em", marginBottom: 4 }}>{m.l}</div>
                      <div style={{ fontSize: 22, fontWeight: 800, fontFamily: "'IBM Plex Mono', monospace",
                        color: COLORS.slate900 }}>{m.v} <span style={{ fontSize: 12 }}>{m.u}</span></div>
                      <div style={{ fontSize: 11, color: COLORS.slate, marginTop: 4 }}>{m.desc}</div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <Card>
                ...
                {[
                  { l: "Solid Solution", v: Math.min(100, (normComp.Ni * 3 + normComp.Mn * 4 + normComp.Si * 8)), color: COLORS.blue },
                  { l: "Grain Refinement", v: Math.min(100, (normComp.V * 120 + normComp.Ti * 80)), color: COLORS.emerald },
                  { l: "Precipitation", v: Math.min(100, normComp.C * 300 + normComp.V * 80), color: COLORS.indigo },
                  { l: "Martensite", v: results.vfMartensite * 100, color: COLORS.rose },
                ].map(m => (
                  <div key={m.l} style={{ marginBottom: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: COLORS.slate700 }}>{m.l}</span>
                      <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 700 }}>
                        {Math.round(m.v)}%
                      </span>
                    </div>
                    <div style={{ height: 8, borderRadius: 99, background: COLORS.slate200, overflow: "hidden" }}>
                    ...
                    return (
                      <div key={el} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                        <span style={{ width: 28, fontSize: 12, fontWeight: 700,
                          fontFamily: "'IBM Plex Mono', monospace", color: COLORS.slate900 }}>{el}</span>
                        <div style={{ flex: 1, height: 8, borderRadius: 99, background: COLORS.slate200, overflow: "hidden" }}>
                          <div style={{ height: "100%", width: `${pct}%`, background: COLORS.emerald, borderRadius: 99 }} />
                        </div>
                        <span style={{ fontSize: 11, color: COLORS.slate, width: 36, textAlign: "right",
                          fontFamily: "'IBM Plex Mono', monospace" }}>{pct.toFixed(0)}%</span>
                      </div>
                    );
                  })}
                <div style={{ marginTop: 12, padding: "10px 14px", background: COLORS.slate100, borderRadius: 8,
                  display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: COLORS.slate700 }}>Total Cost</span>
                  <span style={{ fontSize: 16, fontWeight: 900, fontFamily: "'IBM Plex Mono', monospace",
                    color: COLORS.slate900 }}>${results.cost.toFixed(2)}<span style={{ fontSize: 11, color: COLORS.slate }}>/kg</span></span>
                </div>
              </Card>
            </div>
          </div>
      ...
      {/* ── FOOTER ── */}
      <div style={{ borderTop: `1px solid ${COLORS.slate200}`, padding: "14px 32px",
        display: "flex", justifyContent: "space-between", alignItems: "center",
        background: COLORS.white, marginTop: 16, fontSize: 11, color: COLORS.slate }}>
        <div>AlloyOS Research Platform · Koistinen-Marburger · Dearden-O'Neill · Hall-Petch · Gibbs Phase Rule</div>
        <div style={{ display: "flex", gap: 16 }}>
          <span>CE: <strong style={{ color: COLORS.slate900 }}>{results.CE.toFixed(3)}</strong></span>
          <span>YS: <strong style={{ color: COLORS.slate900 }}>{results.YS} MPa</strong></span>
          <span>UTS: <strong style={{ color: COLORS.slate900 }}>{results.UTS} MPa</strong></span>
          <span>ρ: <strong style={{ color: COLORS.slate900 }}>{results.density.toFixed(3)} g/cm³</strong></span>
        </div>
      </div>
    </div>
  );
